...
- Facebook/Instagram/Threads/TikTok retain expand → preview → explicit confirmation.
  Facebook uses the original synchronous pre-queue save path; Instagram, Threads, and
  TikTok reuse the confirmed structured extraction in the worker rather than run twice.
  TikTok captions are parsed from the page's hydration-state JSON (`ingest/tiktok.ts`).
